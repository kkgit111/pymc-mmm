...
def objective_distribution(
    ...
    for channel, budget, weight in zip(channels, x, weights):
        if method == "michaelis-menten":
            L, k = parameters[channel]
            sum_contributions += michaelis_menten(budget, L, k) * weight
...
def optimize_budget_distribution(
    ...
    result = minimize(
        lambda x: objective_distribution(x, method, channels, weights, parameters),
        initial_guess,
        method="SLSQP",
        bounds=bounds,
